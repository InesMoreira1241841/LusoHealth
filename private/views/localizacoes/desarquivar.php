...
if (!$id_desencriptado || !is_numeric($id_desencriptado)) {
    ...
    header("Location: localizacoes.php?modo=arquivados");
    exit;
}
...
try {
    ...
    $sql = "UPDATE localizacoes SET arquivado = 0 WHERE id = :id";
    $stmt = $ligacao->prepare($sql);
    $stmt->execute([':id' => $id_desencriptado]);

    $_SESSION['success_message'] = "A localização foi restaurada com sucesso.";
